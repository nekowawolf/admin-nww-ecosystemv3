...
interface CommunitySubmissionsTableProps {
...
export default function CommunitySubmissionsTable({
  data,
  loading,
  error,
  onDelete,
  title = 'Community Submissions',
  subtitle = 'Review and manage incoming community submissions'
}: CommunitySubmissionsTableProps) {
  ...
  const filteredData = useMemo(() => {
    return safeData.filter(item =>
      (item.community_link?.toLowerCase() || '').includes(search.toLowerCase()) ||
                  ...
                  <tr key={index} className="border-t border-border-divider">
                    <td className="px-6 py-2">
                      {item.community_link ? (
                        ...
                        >
                          {item.community_link.length > 40 ? item.community_link.slice(0, 40) + '...' : item.community_link}
                        </a>
                      ) : (
                        'N/A'
                      )}
