...
import { useEditGuild } from '@/hooks/guild/useEditGuild'
...
export default function EditGuildForm({ id }: { id: string }) {
  useAuthGuard()
  const router = useRouter()
  const { isSubmitting, initialData, submitEditGuild } = useEditGuild(id)
  const [formData, setFormData] = useState<GuildRequest>({
    name: '',
    description: '',
    image_url: '',
    website: '',
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (['twitter', 'instagram', 'discord', 'youtube', 'github'].includes(name)) {
    ...
    <div className="space-y-12 mt-6 sm:mt-0">
      <div className="text-center sm:text-left">
        ...
        <p className="text-xs sm:text-sm text-secondary">
          Update an existing Guild listing
        </p>
      </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 gap-6">
              ...
              <div className="flex justify-end gap-4 pt-6 border-t border-border-divider">
                <button
                  type="button"
                  onClick={() => router.push('/guild-menu/dashboard/guild-list')}
                ...
                >
                  {isSubmitting ? 'Updating...' : 'Update Guild'}
                </button>
              </div>
            </div>
          </form>
